...
chartTitle = "Total Gamma: $" + str("{:.2f}".format(df['TotalGamma'].sum())) + " Bn per 1% NDX Move"
...
plt.axvline(x=spotPrice, color='r', lw=1, label="NDX Spot: " + str("{:,.0f}".format(spotPrice)))
plt.legend()
plt.show()
...
chartTitle = "Gamma Exposure Profile, NDX, " + todayDate.strftime('%d %b %Y')
plt.title(chartTitle, fontweight="bold", fontsize=20)
